...
export const useTranslationState = ({ 
  ...
  const lastInputChangeTimeRef = useRef<number>(Date.now());
  ...
  const addToTranslationHistory = (sourceText: string, translatedText: string, isComplete: boolean = true) => {
    if (sourceText && translatedText && translatedText !== "[翻译失败]") {
      setTranslationHistory(prev => {
        ...
        // 若最新一条记录与新源文本互为前缀/包含，则覆盖最新记录
        if (prev.length > 0) {
          const latest = prev[0];
          const aInB = sourceText.startsWith(latest.sourceText) || latest.sourceText.startsWith(sourceText);
          if (aInB) {
            const updated = [
              {
                sourceText,
  ...
  // setSourceText 包装，自动更新时间戳
  const setSourceTextWithTime = (text: string) => {
    console.log('[useTranslationState] setSourceTextWithTime called:', text);
    setSourceText(text);
    lastInputChangeTimeRef.current = Date.now();
  };

  // 包装 setTranslatedText，加日志
  const setTranslatedTextWithLog = (text: string) => {
    console.log('[useTranslationState] setTranslatedTextWithLog called:', text);
  ...
  return {
    // 状态
    sourceText,
    setSourceText: setSourceTextWithTime,
    translatedText,
    setTranslatedText: setTranslatedTextWithLog,
    sourceLanguage,
    setSourceLanguage,
    targetLanguage,
    setTargetLanguage,
